...
currentUser=$(scutil --get ConsoleUser | awk '{print $1}')
echo "$currentUser"

osascript -e 'display dialog "* * * * * * * * * * * * * * * W A R N I N G * * * * * * * * * * * * * * * This computer system is the property of Christopher Newport University and it is for authorized use only. By using this system, all users acknowledge notice of, and agree to comply with, the University's Acceptable Use of Computing Resources Policy (6010). Unauthorized or improper use of this system may result in administrative disciplinary action, civil charges/criminal penalties, and/or other sanctions as set forth in the University's Policy. By continuing to use this system you indicate your awareness of and consent to these terms and conditions of use." buttons {"I Agree"} default button 1'
...
#Set the run interval to run every 15 minutes (900 seconds)
...
#Load the daemon (sleep briefly to ensure plist is fully written before loading)
sleep 2
